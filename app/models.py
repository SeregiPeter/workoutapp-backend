...
class Category(Base):
    ...
    exercises = relationship(
        "Exercise", back_populates="category", cascade="all, delete"
    )
...
class Exercise(Base):
    ...
    workouts = relationship(
        "WorkoutExercise", back_populates="exercise", cascade="all, delete"
    )
    challenges = relationship(
        "Challenge", back_populates="exercise", cascade="all, delete"
    )
...
class Challenge(Base):
    ...
    measurement_method = Column(
        SQLAlchemyEnum(
            MeasurementMethodEnum, values_callable=lambda x: [e.value for e in x]
        ),
        nullable=False,
    )
    exercise_id = Column(
